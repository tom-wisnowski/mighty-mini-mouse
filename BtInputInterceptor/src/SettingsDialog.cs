...
public class SettingsDialog : Form
{
    ...
    public SettingsDialog(TrayApplication app, AppConfig config)
    {
        _app = app;
        _config = config;
    ...
    {
        Text = "BT Input Interceptor — Settings";
    ...
    private void ToggleRecording(object? sender, EventArgs e)
    {
        ...
        else
        {
            ...
            _app.StartRecording(inputKey =>
            {
                if (InvokeRequired)
                    BeginInvoke(() => OnInputRecorded(inputKey));
                else
                    OnInputRecorded(inputKey);
            });
        }
    }

    private void OnInputRecorded(string inputKey)
    {
        _inputKeyBox.Text = inputKey;
        _statusLabel.Text = $"Captured: {inputKey}. Now click the 'Maps to' box and press your keys.";
    ...
    protected override bool ProcessCmdKey(ref Message msg, Keys keyData)
    {
        ...
        if (_isRecording)
        {
            ...
            if (keyName != null)
            {
                Debug.WriteLine($"[BtInput][RECORD-FORM] ✓ Captured key via ProcessCmdKey: Key.{keyName}");
                OnInputRecorded($"Key.{keyName}");
                return true;
            }
        }
    ...
    protected override void OnKeyDown(KeyEventArgs e)
    {
        ...
        if (_isRecording)
        {
            ...
            if (keyName != null)
            {
                Debug.WriteLine($"[BtInput][RECORD-FORM] ✓ Captured key via OnKeyDown: Key.{keyName}");
                OnInputRecorded($"Key.{keyName}");
                e.Handled = true;
    ...
    private void AddMapping(object? sender, EventArgs e)
    {
        ...
        Debug.WriteLine($"[BtInput][Settings] Added gesture: {name} ({inputKey} → {keystroke})");
    }
    ...
    private void SaveAndClose(object? sender, EventArgs e)
    {
        try
        {
            ...
            Debug.WriteLine($"[BtInput][Settings] Saved {_gestures.Count} gestures to config.json");
