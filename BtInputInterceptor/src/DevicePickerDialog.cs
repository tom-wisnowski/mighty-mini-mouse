...
public class DevicePickerDialog : Form
{
    ...
    private readonly string? _currentDevicePath;
    ...
    public DevicePickerDialog(RawInputManager rawInputManager, string? currentDevicePath)
    {
    ...
    private void InitializeComponents()
    {
        ...
        _refreshBtn.Click += (_, _) =>
        {
            ...
            Debug.WriteLine("[BtInput][PICKER] Device list refreshed manually");
        };
        ...
        bottomPanel.SetColumnSpan(_statusLabel, 2);
        bottomPanel.Controls.Add(_selectBtn, 2, 0);
        bottomPanel.Controls.Add(_cancelBtn, 3, 0);
    ...
    private void PopulateDevices()
    {
        ...
        _allMice = allDevices.Where(d => d.Type == 0).ToList();

        Debug.WriteLine($"[BtInput][PICKER] Enumerated {allDevices.Count} total devices, {_allMice.Count} mice");
        foreach (var m in _allMice)
            Debug.WriteLine($"[BtInput][PICKER]   Mouse: handle={m.Handle}, name={m.FriendlyName}, path={m.DevicePath}");
        ...
        {
            string vidPid = ExtractVidPid(mouse.DevicePath) ?? mouse.DevicePath;
            ...
            string label = $"  {mouseIndex}. {mouse.FriendlyName}";
            ...
            Debug.WriteLine($"[BtInput][PICKER]   Display row {itemIndex}: {mouse.FriendlyName} vidpid={vidPid}");
    ...
    {
        // Standard USB format: VID_xxxx&PID_xxxx
        var usbMatch = Regex.Match(path, @"VID_([0-9A-Fa-f]{4})&PID_([0-9A-Fa-f]{4})",
            RegexOptions.IgnoreCase);
        if (usbMatch.Success)
            return $"VID_{usbMatch.Groups[1].Value}&PID_{usbMatch.Groups[2].Value}".ToUpperInvariant();

        // Bluetooth format: VID&02248a_PID&8266 (variable length hex)
        var btMatch = Regex.Match(path, @"VID&([0-9A-Fa-f]+)_PID&([0-9A-Fa-f]+)",
            RegexOptions.IgnoreCase);
        if (btMatch.Success)
        {
    ...
    private void StartActivityMonitor()
    {
        ...
        Debug.WriteLine($"[BtInput][PICKER] Activity monitor started. _itemVidPid entries:");
        foreach (var kvp in _itemVidPid)
            Debug.WriteLine($"[BtInput][PICKER]   row[{kvp.Key}] = {kvp.Value}");
    }
    ...
    private void ActivityTimer_Tick(object? sender, EventArgs e)
    {
        ...
        // 1. Get the most recent raw input handle
        var handle = _rawInputManager.GetLastSeenDeviceHandle();
        ...
        if (handle != IntPtr.Zero)
        {
            ...
            if (!_handleToVidPid.TryGetValue(handle, out var vidPid))
            {
                ...
                vidPid = ExtractVidPid(devicePath);
                _handleToVidPid[handle] = vidPid;
                Debug.WriteLine($"[BtInput][PICKER] New handle {handle} → path={devicePath}");
                Debug.WriteLine($"[BtInput][PICKER]   → VID/PID: {vidPid ?? "(none)"}");
            }
            ...
            if (vidPid != null)
            {
                ...
                if (isNew)
                {
                    ...
                    Debug.WriteLine($"[BtInput][PICKER] ★ Activity: vidpid={vidPid} handle={handle}");
                }
            }
            else if (verbose)
            {
                Debug.WriteLine($"[BtInput][PICKER] Tick #{_tickCount}: handle={handle} has no VID/PID");
            }
        }
        else if (verbose)
        {
            Debug.WriteLine($"[BtInput][PICKER] Tick #{_tickCount}: handle=0 (no raw input yet)");
        }
        ...
        foreach (var kvp in _itemVidPid)
        {
            ...
            if (_lastActivity.TryGetValue(vidPid, out var lastSeen) &&
                (now - lastSeen).TotalSeconds < ACTIVITY_DURATION_SECS)
            {
        ...
        if (verbose)
            Debug.WriteLine($"[BtInput][PICKER] Active items: [{string.Join(", ", newActiveItems)}] (was: [{string.Join(", ", _activeItems)}])");
        ...
        if (!newActiveItems.SetEquals(_activeItems))
        {
            ...
            foreach (var idx in added)
                Debug.WriteLine($"[BtInput][PICKER] ● DOT ON  row {idx} (vidpid={_itemVidPid.GetValueOrDefault(idx, "?")})");
            foreach (var idx in removed)
                Debug.WriteLine($"[BtInput][PICKER] ○ DOT OFF row {idx}");
            ...
            if (_deviceList.Items.Count > 0)
            {
                ...
                Debug.WriteLine($"[BtInput][PICKER] Invalidated dot column rect: {dotColumnRect}");
            }
        }
    }
    ...
    {
        _selectBtn.Enabled = _deviceList.SelectedItems.Count > 0;
    }
    ...
    private void ConfirmSelection()
    {
        ...
        if (selected.Tag is DeviceInfo device)
        {
            ...
            Debug.WriteLine($"[BtInput][PICKER] Selected: {device.FriendlyName} ({device.DevicePath})");
        }
        else
        {
            SelectedDevice = null;
            AllDevicesSelected = true;
            Debug.WriteLine("[BtInput][PICKER] Selected: All Mice (no filter)");
        }
    ...
    protected override void OnFormClosing(FormClosingEventArgs e)
    {
        ...
        _activityTimer = null;
        Debug.WriteLine("[BtInput][PICKER] Dialog closing, activity monitor stopped");
